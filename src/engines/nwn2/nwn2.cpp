...
void NWN2Engine::run(const Common::UString &target) {
	...
	CameraMan.setPosition(glm::vec3(0.0, 2.0, 0.0));

	Graphics::Aurora::Model *model = loadModelObject("plc_br_mulsantirhouse05");

	model->setPosition(glm::vec3(0.0, 20.0, 0.0));
	model->show();
		...
		while (EventMan.pollEvent(event)) {
			if (event.type == Events::kEventKeyDown) {
				if      (event.key.keysym.sym == SDLK_UP)
					...
					const glm::vec3 orient = CameraMan.getOrientation();

					CameraMan.setOrientation(glm::vec3(0.0, orient.y, orient.z));
				}
			}
		}
